...
async function parseResponse(response, path) {
  ...
  if (!response.ok) {
    const detail = typeof payload === "object" && payload?.detail ? JSON.stringify(payload.detail) : text2;
...
async function fetchMessages(sessionId) {
  ...
  return (data.messages || []).map((m2) => {
    ...
    if (role === "user") t2 = "user";
      ...
      try {
        const p2 = JSON.parse(content2);
        content2 = p2.content || content2;
        extra.exitCode = p2.is_error ? 1 : 0;
      } catch {
      }
    }
    return { id: m2.id, type: t2, content: content2, createdAt: m2.created_at, agentName: "", agentAvatar: "", ...extra };
  });
}
async function sendMessage(sessionId, content2) {
  await api().post(`/api/sessions/${sessionId}/messages`, { role: "user", content: content2, run: true });
}
...
async function pauseSchedule(name2) {
  return api().post(`/api/schedules/${name2}/pause`);
}
async function resumeSchedule(name2) {
  return api().post(`/api/schedules/${name2}/resume`);
}
async function deleteSchedule(name2) {
  return api().delete(`/api/schedules/${encodeURIComponent(name2)}`);
}
...
function adaptEvent(event) {
  ...
  switch (etype) {
    ...
    case "thinking_delta": {
      ...
      return { id: `thinking-${id2}`, type: "thinking", content: text2, createdAt: now };
    }
    case "assistant_text_delta": {
      const text2 = payload.text || "";
      if (!text2.trim()) return null;
      return { id: `delta-${id2}`, type: "assistant", agentName: "", agentAvatar: "🤖", content: text2, createdAt: now };
    }
    case "assistant_message": {
      const text2 = payload.text || payload.content || JSON.stringify(payload.content_blocks || payload);
      if (!text2 || text2.startsWith("[{")) return null;
      return { id: id2, type: "assistant", agentName: "", agentAvatar: "🤖", content: text2, createdAt: now };
    }
    ...
    case "tool_result": {
      const content2 = typeof payload.content === "string" ? payload.content.slice(0, 3e3) : JSON.stringify(payload.content).slice(0, 3e3);
      return {
...
const useMessageStore = create$1((set, get) => ({
  messagesBySession: {},
  runningBySession: {},
  ...
  appendUserMessage: async (sessionId, content2) => {
    const userMsg = { id: `user-${Date.now()}`, type: "user", content: content2, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
    ...
    try {
      await sendMessage(sessionId, content2);
    } catch (e) {
...
function SessionTreeItem({ session }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "group flex items-center gap-1", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "hidden group-hover:flex items-center gap-0.5 shrink-0", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleCopyId, className: "rounded p-1 text-app-muted hover:bg-app-hover hover:text-app-text", title: "Copy session ID", children: copied ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { size: 12, className: "text-green-400" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 12 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleDeleteClick, className: "rounded p-1 text-red-400/50 hover:text-red-400 hover:bg-red-900/20", title: "Delete session", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 12 }) })
      ] })
    ] }),
    showConfirm && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/60", onClick: () => setShowConfirm(false), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-80 rounded-xl border border-red-500/30 bg-app-panel p-5 shadow-2xl", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mb-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 18, className: "text-red-400" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-sm font-semibold text-app-text", children: "Delete Session" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-app-secondary mb-1", children: "Are you sure you want to delete" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm font-medium text-app-text truncate mb-4", children: [
        '"',
        session.name,
        '"?'
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-app-muted mb-4", children: "This will permanently delete all messages and cannot be undone." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex justify-end gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => setShowConfirm(false), className: "rounded-lg border border-app-border px-4 py-2 text-sm text-app-muted hover:bg-app-hover", children: "Cancel" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleConfirmDelete, disabled: deleting, className: "rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-500 disabled:opacity-50", children: deleting ? "Deleting..." : "Delete" })
      ] })
    ] }) })
  ] });
}
function CreateSessionModal({ agentId, agentName, open, onClose }) {
  const [name2, setName] = reactExports.useState("");
  const [cwd2, setCwd] = reactExports.useState("");
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    try {
      const projects = await (await __vitePreload(async () => {
        const { fetchProjects: fetchProjects2 } = await Promise.resolve().then(() => client);
        return { fetchProjects: fetchProjects2 };
      }, true ? void 0 : void 0, import.meta.url)).fetchProjects();
      const projectId = projects[0]?.id;
      if (!projectId) throw new Error("No project found");
      const api2 = await __vitePreload(() => Promise.resolve().then(() => client), true ? void 0 : void 0, import.meta.url);
      await api2.createSession(projectId, agentId, name2 || `${agentName} session`, cwd2 || void 0);
      await loadAgents();
      onClose();
    } catch (e) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/60", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-96 rounded-xl border border-app-border bg-app-panel p-5 shadow-2xl", onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "text-sm font-semibold text-app-text", children: [
        "New ",
        agentName,
        " Session"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onClose, className: "rounded-md p-1 text-app-muted hover:bg-app-hover hover:text-app-text", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-app-muted mb-1", children: "Session name" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            placeholder: cwd2 || `${agentName} working directory`,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-xs text-app-muted mb-1", children: "Working directory" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            type: "text",
    ...
    error && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3 text-xs text-red-400", children: error }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-4 flex justify-end gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onClose, className: "rounded-lg border border-app-border px-4 py-2 text-sm text-app-muted hover:bg-app-hover", children: "Cancel" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleCreate, disabled: creating, className: "rounded-lg bg-app-accent px-4 py-2 text-sm font-medium text-white hover:bg-violet-600 disabled:opacity-50", children: creating ? "Creating..." : "Create" })
    ] })
  ] }) });
}
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "opacity-40 hover:opacity-100 text-app-muted", onClick: (e) => {
            e.stopPropagation();
            setShowCreate(true);
...
function ConnectorsSection() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(SidebarSection, { title: "Connectors", action: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "rounded-md p-1 text-app-muted hover:bg-app-hover hover:text-app-text", onClick: () => setView("connectors"), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 14 }) }), children: [
    connectors.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "flex w-full items-center rounded-lg px-2 py-2 text-left text-sm text-app-muted hover:bg-app-hover hover:text-app-text", onClick: () => setView("connectors"), children: "No connectors" }),
    connectors.map((c) => /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm hover:bg-app-hover", onClick: () => setView("connectors"), children: [
...
const useRunnerStore = create$1((set) => ({
  runners: [],
  loading: false,
  error: null,
  loadRunners: async () => {
...
function UserMessageBubble({ message }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-end", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-1 flex items-center justify-end gap-2 text-xs text-app-muted opacity-0 hover:opacity-100 transition-opacity", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatTime$1(message.createdAt) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleCopy, className: "rounded p-0.5 hover:bg-app-hover hover:text-app-text", title: "Copy", children: copied ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { size: 12, className: "text-green-400" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 12 }) })
    ] })
  ] }) });
}
function ok$1() {
...
function AssistantMessageBubble({ message }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-start", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full max-w-4xl", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rounded-xl border border-app-border bg-app-panel px-4 py-3 text-sm leading-6 text-app-text prose prose-invert prose-sm max-w-none", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Markdown, { children: message.content }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-1 flex items-center gap-2 text-xs text-app-muted opacity-0 hover:opacity-100 transition-opacity", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleCopy, className: "rounded p-0.5 hover:bg-app-hover hover:text-app-text", title: "Copy", children: copied ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { size: 12, className: "text-green-400" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 12 }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatTime(message.createdAt) })
    ] })
  ] }) });
}
function ToolInvocationCard({ message }) {
  ...
  const handleCopy = () => {
    const text2 = message.command || message.summary;
    navigator.clipboard.writeText(text2).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2e3);
    });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-app-border bg-app-card overflow-hidden group", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          expanded ? /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { size: 15, className: "text-app-muted shrink-0" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { size: 15, className: "text-app-muted shrink-0" }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "min-w-0 flex-1 truncate font-mono text-sm text-app-secondary", children: expanded ? message.summary || message.command : firstLine }),
    ...
    expanded && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pb-3 pl-12 relative", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "font-mono text-sm text-app-secondary whitespace-pre-wrap break-all max-h-48 overflow-y-auto", children: message.command || message.summary }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          onClick: handleCopy,
          ...
          children: copied ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { size: 14, className: "text-green-400" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 14 })
        }
      )
    ] })
  ] });
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-app-border bg-app-card overflow-hidden group", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          expanded ? /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { size: 15, className: "mt-0.5 text-app-muted shrink-0" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { size: 15, className: "mt-0.5 text-app-muted shrink-0" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `rounded px-1.5 py-0.5 text-xs font-bold uppercase shrink-0 ${message.exitCode === 0 ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`, children: "Result" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "min-w-0 flex-1 overflow-hidden font-mono text-sm leading-6 text-app-secondary whitespace-pre-wrap", children: expanded ? message.content : firstLine })
        ]
      }
    ),
    expanded && hasMore && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pb-3 pl-12 relative", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "font-mono text-sm text-app-secondary whitespace-pre-wrap break-all max-h-96 overflow-y-auto", children: lines.slice(1).join("\n") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          onClick: handleCopy,
...
function Composer() {
  ...
  function send() {
    ...
    if (!content2 || !selectedSessionId) return;
    appendUserMessage(selectedSessionId, content2);
    setValue("");
  }
...
function ConnectorsPanel() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "flex min-w-0 flex-1 flex-col bg-app-bg", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "overflow-y-auto p-5 space-y-3", children: [
      loading && !connectors.length && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-app-muted", children: "Loading connectors..." }),
      error && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rounded-xl border border-red-800 bg-red-900/20 p-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-red-400", children: error }) }),
      !loading && connectors.length === 0 && !error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-app-border bg-app-panel p-6 text-center", children: [
